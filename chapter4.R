...
# リスト 4-1-(4)
## c = np.array([[1, 2], [3, 4]])
c <- as.array(matrix(c(1, 2, 3, 4), nrow = 2, ncol = 2, byrow = TRUE))
## print(c)
print(c)
...
b <- array(c(1, 3), dim = c(1, 2))
## c = np.array([4, 2])
c <- array(c(4, 2), dim = c(1, 2))
## print(b @ c)
print(as.vector(b) %*% as.vector(c))
...
df <- data.frame(array(NA, dim = c(w0_n * w1_n, 3))) |>
  rename(ww0 = 'X1',
         ww1 = 'X2',
         f_num = 'X3')
t <- 1
...
A <- as.array(matrix(1:6, nrow = 2, ncol = 3, byrow = TRUE))
## print(A)
print(A)
...
B <- as.array(matrix(7:12, nrow = 2, ncol = 3, byrow = TRUE))
## print(B)
print(B)
...
A <- as.array(matrix(c(1:3, -1:-3), nrow = 2, ncol = 3, byrow = TRUE))
...
A <- as.array(matrix(1:9, nrow = 3, ncol = 3, byrow = TRUE))
## I = np.identity(3)
...
# リスト 4-4-(12)
## A = np.array([[1, 2], [3, 4]])
A <- as.array(matrix(1:4, nrow = 2, ncol = 2, byrow = TRUE))
...
tibble(a    = c(x, x, x),
       b    = c(y, y2, y3),
       type = rep(c('1', '2', '3'), each = length(x))) |>
  ggplot(aes(x = a, y = b, color = type)) +
  geom_line() +
...
ggplot() +
  geom_line(data    = tibble(a = x, b = y),
            mapping = aes(x = a, y = b),
            color   = 'black') +
  geom_line(data    = tibble(c = x2, d = y2),
            mapping = aes(x = c, y = d),
            color   = 'cornflowerblue') +
  geom_line(data     = tibble(e = x, f = x),
            mapping  = aes(x = e, y = f),
            linetype = 'dashed') +
  labs(x = element_blank(), y = element_blank()) +
  xlim(-8, 8) + ylim(-8, 8) +
...
ggplot() +
  geom_line(data    = tibble(a = x, b = y),
            mapping = aes(x = a, y = b),
            color   = 'black') +
  geom_line(data    = tibble(c = x, d = logy),
            mapping = aes(x = c, y = d),
            color   = 'cornflowerblue') +
  labs(x = element_blank(), y = element_blank()) +
  xlim(-4, 4) + ylim(-4, 8) +
...
ggplot() +
  geom_line(data     = tibble(a = x, b = y),
            ...
            color    = 'gray',
            linetype = 'dashed') +
  geom_line(data    = tibble(c = x, d = dy),
            mapping = aes(x = c, y = d)) +
  labs(x = element_blank(), y = element_blank()) +
  xlim(-4, 4) + ylim(-1, 8) +
...
y <- softmax(xx0, xx1, 1)
y <- data.frame(array(NA, dim = c(x0_n * x1_n, 4))) |>
  rename(x0 = 'X1',
         x1 = 'X2',
         y0 = 'X3',
         y1 = 'X4')
...
for (i in 1:x0_n) {
  for (j in 1:x1_n) {
    y[t, 1] <- xx0[i, j]
    y[t, 2] <- xx1[i, j]
    y[t, 3] <- softmax(xx0[i, j], xx1[i, j], 1)[1]
    y[t, 4] <- softmax(xx0[i, j], xx1[i, j], 1)[2]
    t <- t + 1
  }
}
...
## plt.show()
ggplot() +
  geom_line(data    = tibble(a = x, b = gauss(0, 1, 1, x)),
            mapping = aes(x = a, y = b),
            color   = 'black') +
  geom_line(data    = tibble(c = x, d = gauss(2, 2, 0.5, x)),
            mapping = aes(x = c, y = d),
            color   = 'gray') +
  labs(x = element_blank(), y = element_blank()) +
